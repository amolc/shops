app.controller('studentCtrl', function($scope, $http, $window, $location, $sce, $timeout, store) {
    ...
    $scope.list = function(req, res) {
        $http.get($scope.baseurl + 'students/')
    ...
    $scope.add = function(req, res) {
        ...
        if (typeof $scope.data.id == 'undefined') {
            alert($scope.data.id);
            $http.post($scope.baseurl + 'students/', $scope.data)
                .success(function(res) {
                    if (res.status == 'false') {} else {
